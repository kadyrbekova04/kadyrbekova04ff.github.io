...
function initBattery() {
    ...
    navigator.getBattery().then((batt) => {
        updateBattery = () =>{
            ...
            if(level == 100) {
                batteryStatus.innerHTML = `Батарея полная <i class="ri-battery-2-fill green-color"></i>`;
                batteryLiquid.style.height = "103%";
            }else if (level <= 20 & !batt.charging){
                batteryStatus.innerHTML = `Мало заряда <i class="ri-plug-line animated-red animated-red"></i>`
            ...
            if(level <=20){
                ...
                batteryLiquid.classList.remove("gradient-color-green", "gradient-color-red", "gradient-color-red");
            }else{
                batteryLiquid.classList.add("gradient-color-green");
                batteryLiquid.classList.remove("gradient-color-red", "gradient-color-red", "gradient-color-red");
            }
        }
        updateBattery();
